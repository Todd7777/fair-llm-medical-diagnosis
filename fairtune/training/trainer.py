...
This module implements specialized training utilities for Fair LoRA (FairTune) that balance
performance and fairness across demographic groups.
"""
...
from ..models.fair_lora import FairLoRAModel, FairLoRAConfig
from ..metrics.fairness import EquityScaledLoss, compute_fairness_metrics


class FairLoRATrainer:
    """
    Trainer for Fair LoRA fine-tuning with equity-focused objectives.
    """
    ...
    def __init__(
        ...
        wandb_project: str = "fair-lora-medical",
        ...
        self.equity_loss = EquityScaledLoss(
    ...
    def train(self):
        """Train the model with Fair LoRA."""
        global_step = 0
        ...
        for epoch in range(self.num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                demographics = batch.get(self.demographic_key, ["default"] * batch["input_ids"].shape[0])
                ...
                with torch.cuda.amp.autocast() if self.mixed_precision else nullcontext():
                    ...
                    # Compute loss with equity scaling
                    loss = self.equity_loss(
                        outputs.logits,
                        labels,
                        demographics
                    )
                ...
                if self.use_wandb:
                    wandb.log({
                        ...
                        "train/lr": self.lr_scheduler.get_last_lr()[0] if self.lr_scheduler else 0,
                    }, step=global_step)
                ...
                if global_step % self.save_steps == 0:
                    self.save_checkpoint(os.path.join(self.checkpoint_dir, f"checkpoint-{global_step}"))
            ...
            # Save checkpoint
            self.save_checkpoint(os.path.join(self.checkpoint_dir, f"checkpoint-epoch-{epoch + 1}"))
    ...
    def load_checkpoint(self, path: str):
        ...
        if self.lr_scheduler is not None and checkpoint["lr_scheduler_state_dict"] is not None:
...
def create_fair_lora_trainer(
    ...
    **kwargs
    ...
    """
    Create Fair LoRA trainer with default settings.
    ...
    Args:
        ...
        equity_weight: Weight for the equity component in the loss
        **kwargs: Additional arguments for FairLoRATrainer
