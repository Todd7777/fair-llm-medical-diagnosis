#!/usr/bin/env python
"""
Unit tests for Fair LoRA implementation.
"""
...
from fairtune.models.fair_lora import (
    FairLoRAConfig,
    FairLoRALayer,
    FairLoRAModel,
    prepare_fair_lora_config
)
...
class TestFairLoRAConfig(unittest.TestCase):
    """Test FairLoRAConfig class."""
    ...
    def test_prepare_fair_lora_config(self):
        """Test prepare_fair_lora_config function."""
        config_dict = {
            "r": 16,
            "alpha": 32.0,
            "dropout": 0.2,
        ...
        config = prepare_fair_lora_config(config_dict)
...
class TestFairLoRALayer(unittest.TestCase):
    """Test FairLoRALayer class."""
    ...
    def test_init(self):
        ...
        for group in self.demographic_groups:
            self.assertEqual(getattr(self.layer, f"lora_S_{group}").shape, (self.r, self.r))
...
class TestFairLoRAModel(unittest.TestCase):
    """Test FairLoRAModel class."""
    
    def setUp(self):
        """Set up test case."""
        self.base_model = MockModel()
        self.demographic_groups = ["group1", "group2"]
        self.config = FairLoRAConfig(
            r=4,
            alpha=8.0,
            dropout=0.1,
            demographic_groups=self.demographic_groups,
    ...
    def test_init(self):
        """Test initialization."""
        # Check if Fair LoRA layers were added
        self.assertTrue(hasattr(self.model.base_model.linear1, "fair_lora_layer"))
        self.assertTrue(hasattr(self.model.base_model.linear2, "fair_lora_layer"))
        
        # Check Fair LoRA layer configurations
        self.assertEqual(self.model.base_model.linear1.fair_lora_layer.r, self.config.r)
        self.assertEqual(self.model.base_model.linear1.fair_lora_layer.demographic_groups, self.demographic_groups)
    ...
    def test_get_fair_lora_parameters(self):
        """Test get_fair_lora_parameters method."""
        # Get Fair LoRA parameters
        fair_lora_params = list(self.model.get_fair_lora_parameters())
        
        # Check if parameters are returned
        self.assertTrue(len(fair_lora_params) > 0)
        
        # Check if all parameters are from Fair LoRA layers
        for param in fair_lora_params:
            self.assertTrue("fair_lora_layer" in param[0])
