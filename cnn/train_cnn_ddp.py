...
class TrainCnnDdp:
    def __init__(self, process_rank, world_size, use_cuda, args):
        ...
        self.warmup_steps = config[self.name][self.dataset_name]["training"].get(
            "warmup_steps", 0
        )
        ...
        if "warmup_steps" in config[self.name][self.dataset_name]["training"]:
    ...
    def save_model(self, process_rank):
        ...
        path = os.path.join(
            ...
            f"{self.name}_{self.dataset_name}_fine_tuned_last_epoch.pt",
        )
        if process_rank == 0:
            model_to_save = (
                self.model.module
    ...
    def train(self):
        ...
        num_epochs = config[self.name][self.dataset_name]["training"]["epochs"]
